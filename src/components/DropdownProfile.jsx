...
const DropdownProfile = ({ profileDropDown }) => {
    ...
    return (
        <div tabIndex="0" className={`dropdown-content z-[1] menu p-2 shadow bg-base-300 rounded-box w-80 ${profileDropDown ? "": "hidden"}`}>
            <div className="rounded-lg bg-base-300 p-3 drop-shadow-xl divide-y divide-neutral">
                ...
                <div className="pt-2">
                    <button type="button" className="flex items-center space-x-3 py-3 px-4 w-full leading-6 text-lg text-gray-600 focus:outline-none hover:bg-gray-100 rounded-md">
                        ...
                        <span className="text-base" onClick={handleLogout}>Logout</span>
                    </button>
                </div>
            </div>
        </div>
